  ...
  try {
    ...
    if (check) {
      await User.updateOne(
        {
          _id: req.user.id, //We look for the user, we look for it inside the array of search
...
exports.getFriendsPageInfos = async (req, res) => {
  try {
    ...
    //Go to other users field to search for your id in their requests array
    const sentRequests = await User.find({
      requests: mongoose.Types.ObjectId(req.user.id), //It will convert every id we pass in to ObjectId
