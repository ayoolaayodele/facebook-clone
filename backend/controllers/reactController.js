...
exports.getReacts = async (req, res) => {
  try {
    ...
    */
    const newReacts = reactsArray.reduce((group, currReact) => {
      // let key = react["react"];
      const { react } = currReact;
      group[react] = group[react] || [];
      group[react].push(currReact);
      return group;
    }, {});
    ...
    // const check1 = reacts.find((x) => x.postRef.toString() === req.params.id);
    // console.log(check1);
